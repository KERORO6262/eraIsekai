...
#### D. 種族與職業（Stats & Growth）
> **關鍵規則**：種族（Race）與職業（Job）完全獨立，採用「種族基礎級距 + 職業加成」後再 `Clamp(1,5)` 的機制，不做任何綁定限制。

| Key（建議命名） | 型態 | 範圍 | 說明 |
|---|---|---|---|
| CFLAG:CHARA:RACE | SAVEDATA | 1..8 | 種族 ID（Human~Lamia） |
| CFLAG:CHARA:JOB | SAVEDATA | 1..8 | 職業 ID（Paladin~Diviner） |

**種族基礎級距（1~5）**

| 種族 | 速度 | 暴擊 | 物攻 | 魔攻 | 特攻 | 物防 | 魔防 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Human | 2 | 2 | 2 | 2 | 2 | 2 | 2 |
| Elf | 3 | 2 | 1 | 4 | 2 | 1 | 3 |
| Halfling | 3 | 3 | 1 | 1 | 3 | 2 | 3 |
| Dwarf | 1 | 1 | 3 | 1 | 2 | 4 | 3 |
| Orc | 2 | 3 | 4 | 1 | 1 | 3 | 1 |
| Dragonewt | 2 | 2 | 3 | 2 | 4 | 4 | 2 |
| Harpy | 4 | 4 | 2 | 2 | 2 | 1 | 1 |
| Lamia | 2 | 2 | 2 | 3 | 4 | 2 | 3 |

**職業加成（可為負值）**

| 職業 | 速度 | 暴擊 | 物攻 | 魔攻 | 特攻 | 物防 | 魔防 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Paladin | -1 | 0 | 0 | 0 | 0 | +2 | +1 |
| Mage | 0 | 0 | -1 | +2 | +1 | 0 | 0 |
| Merchant | +1 | +1 | 0 | 0 | +1 | 0 | 0 |
| Blacksmith | -1 | 0 | +1 | 0 | 0 | +2 | 0 |
| Hunter | +1 | +2 | 0 | 0 | 0 | 0 | -1 |
| Warrior | 0 | 0 | +2 | -1 | 0 | +1 | 0 |
| Assassin | +2 | +2 | 0 | 0 | 0 | -1 | -1 |
| Diviner | 0 | 0 | 0 | +1 | +2 | -1 | 0 |

**計算流程（Single Source of Truth）**
1. 讀取 `RACE` 對應 7 項級距。
2. 疊加 `JOB` 修正值。
3. 每項級距執行 `LIMIT(value, 1, 5)`。
4. 依下方 BASE 換算規則寫入實際戰鬥數值。
...
| 410 | BTL_SPEED | 0 | 999999 | 15 | 速度（由級距換算） |
| 411 | BTL_CRIT | 0 | 999999 | 5 | 暴擊率 %（由級距換算） |
| 412 | BTL_PATK | 0 | 999999 | 10 | 物攻（由級距換算） |
| 413 | BTL_MATK | 0 | 999999 | 10 | 魔攻（由級距換算） |
| 414 | BTL_SATK | 0 | 999999 | 10 | 特攻（由級距換算） |
| 415 | BTL_PDEF | 0 | 999999 | 10 | 物防（由級距換算） |
| 416 | BTL_MDEF | 0 | 999999 | 10 | 魔防（由級距換算） |

### 3.3 級距換算規則（1~5 -> BASE 實值）
- 攻防屬性（`BTL_PATK/BTL_MATK/BTL_SATK/BTL_PDEF/BTL_MDEF`）：1=5、2=10、3=15、4=20、5=25
- 速度（`BTL_SPEED`）：1=10、2=15、3=20、4=25、5=35
- 暴擊（`BTL_CRIT`，%）：1=3、2=5、3=8、4=12、5=20
