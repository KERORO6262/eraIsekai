...
>
> 入口：`CALL BATTLE_ENTER`（於 `ERB/Battle/BATTLE_MAIN.ERB`）
>
> 核心：
> 1) 以元素槽位（非道具卡）組成回合資源；
...
當前文件作為「傷害公式實作前」的基準規範。建議在下階段於命中技能後，按以下順序接入：

1. 先讀取 `TFLAG:TEMP_BUFF_*` 形成當回合倍率（例如 `1 + TEMP_BUFF_PATK/100.0`）。
2. 依技能 ID 拉取技能威力、命中、屬性標籤。
3. 套用攻防公式並結算 HP/EN。
4. 結算完成後保留既有清理流程，不破壞回合推進邏輯。
