...
玩家可在戰鬥 UI 中啟用 `5`、`10` 或 `20` 回合的自動戰鬥，但在啟動前**必須**先設定一項 AI 策略。系統以 `TFLAG:AUTO_STRATEGY` 與 `TFLAG:AUTO_TURNS_LEFT` 分別管理策略狀態與剩餘回合數：
...
若玩家尚未設定策略而直接啟動自動戰鬥，系統會提示 `請先選擇一項自動戰鬥策略！`，並維持在手動選單，不扣除任何回合。
...
- 直接讀取 `TFLAG:AUTO_STRATEGY` 作為決策來源，而非依賴角色預設權重。
- `激進輸出` 會優先選取物理 / 魔法元素，以提高進攻技能與基礎輸出密度。
- `保守防禦` 會優先選取防禦 / 特殊元素；當 HP 偏低時，亦會提高魔法元素的選取優先度，以支援回復技能觸發。
- 若 Temp 內存在更符合當前策略的元素，且本回合尚未使用暫存槽，AI 會嘗試與不理想手牌交換，以改善本回合可用配方。
- 選牌完成後，系統會印出宣告式戰報，例如 `【自動戰報】第 X 回合：AI 採取 [策略名稱] 策略，選取了 元素列表...`，再交由主戰鬥流程統一結算。
