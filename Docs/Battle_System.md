...
玩家可在戰鬥 UI 中啟用 `5`、`10` 或 `20` 回合的自動戰鬥，但在啟動前**必須**先指定一項「策略意圖」。系統以 `TFLAG:AUTO_STRATEGY` 與 `TFLAG:AUTO_TURNS_LEFT` 分別管理策略意圖與剩餘回合數。
...
由於 `410~413` 已改為策略意圖專用按鍵，手動切換手牌選取會重新映射至 `110~113`，使「手動選牌」與「AI 意圖設定」在 UI 上保持單一語義。

若玩家尚未設定策略意圖而直接啟動自動戰鬥，系統會提示 `請先選擇一項策略意圖！`，並維持在手動選單，不扣除任何回合。

當 `TFLAG:AUTO_TURNS_LEFT > 0` 時，戰鬥 UI 會：
1. 顯示 `【自動戰鬥中... 剩餘 X 回合】`。
2. 先短暫停留，讓玩家閱讀上一個畫面。
3. 呼叫 `@BATTLE_AUTO_PROCESS` 產生選牌結果與簡短自動戰報。
4. 再以既有 `@BATTLE_PROCESS_ACTION` / `@BATTLE_ENEMY_TURN` 推進回合。
5. 回合結束後再次短暫停留，並將 `TFLAG:AUTO_TURNS_LEFT -= 1`。
6. 當剩餘回合歸零時，系統會明確提示「自動戰鬥已結束，恢復手動操作」。

系統提供四種長期穩定的策略意圖：偏好物理、偏好魔法、偏好防禦、偏好特殊。AI 會將與目前策略意圖相符的元素視為最高優先選取目標，並在需要時透過 Temp 交換將對應元素移回主手牌區。

選牌完成後，系統會印出宣告式戰報，例如 `【自動戰報】第 X 回合：AI 執行 [策略意圖：偏好物理]，選取了 元素列表...`，再交由主戰鬥流程統一結算。

勝利與敗北結算皆會停留於結果畫面等待玩家確認（`WAIT`），確保最後一擊、戰利品與敗北訊息都能被完整閱讀後，才返回基地流程。
