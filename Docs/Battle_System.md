...
目前木人樁戰鬥的基準掉落為金錢 `100`、材料 `20`。

### 6.1 結算流程（`@BATTLE_SETTLEMENT_REPORT`）

`@BATTLE_SETTLEMENT_REPORT` 位於 `ERB/Battle/BATTLE_MAIN.ERB`，由 `CORE_LOOP.ERB` 在 `CALL BATTLE_PREPARATION` 返回後立即呼叫。

> **職責邊界**：所有戰鬥 UI、資源結算邏輯、戰報輸出，都集中於 `ERB/Battle/` 領域。`CORE_LOOP.ERB` 只做 `CALL`，不持有任何戰鬥資源計算邏輯。

結算邏輯如下：

1. 若 `RESULT == 1`（勝利）：
   - 將 `TFLAG:BATTLE_LOOT_MONEY` 與 `TFLAG:BATTLE_LOOT_MAT` 累加進 `FLAG:RES_MONEY` / `FLAG:RES_MATERIAL`。
   - **Clamp 防呆**：更新後對 `FLAG:RES_MONEY` 與 `FLAG:RES_MATERIAL` 執行 `MAX(0, 值)` 保護，確保不為負值。
   - 印出包含「勝利結果、耗時回合數、獲得資源」的戰報。
2. 若 `RESULT == 0`（敗北/撤退）：
   - 不發放掉落資源。
   - 印出包含「失敗結果、耗時回合數」的戰報。
3. 結算完成後：
   - 清空 `TFLAG:BATTLE_LOOT_MONEY` 與 `TFLAG:BATTLE_LOOT_MAT`，避免殘留滲入下次結算。
   - `WAIT` 等待玩家確認，再 `RETURN` 回 `CORE_LOOP.ERB`。

此設計使戰鬥模組完整封裝「回傳勝敗 + 暫存掉落 + 資源入帳 + 戰報輸出」的全部責任，`CORE_LOOP.ERB` 的呼叫序列保持為純 `CALL` 委派，兩者邊界穩定且可長期擴充。
