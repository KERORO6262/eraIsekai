...
├─ ERB/
│  ├─ Base/
│  │  ├─ BASE_MANAGEMENT.ERB   協調入口：初始化、主選單路由、共用工具
│  │  ├─ BASE_LIFECYCLE.ERB    生命週期 Tick：晨間/晚間結算、晨報輸出
│  │  ├─ BASE_UI.ERB           UI 輸出：HUD 與住宿 NPC 列表
│  │  ├─ BASE_TRAIN.ERB        訓練室選單 + @SYS_TRAIN_APPLY
│  │  ├─ BASE_WORK.ERB         工坊選單 + 生產佇列操作
│  │  ├─ BASE_LOUNGE.ERB       休息室選單 + 互動前置檢查
│  │  ├─ BASE_INTERACT.ERB     互動系統（分派、指令、公式、回饋）
│  │  └─ BASE_ROOM.ERB         房間管理選單 + @SYS_ROOM_MOVE_IN/OUT
│  ├─ Battle/
│  │  └─ BATTLE_MAIN.ERB
│  └─ System/
│     ├─ ERH.ERH
...
### `ERB/Base/`
**職責：基地管理域（SRP 拆分為 8 個模組）。**

| 模組檔案 | 核心職責 | 主要函式 |
|---|---|---|
| `BASE_MANAGEMENT.ERB` | 協調入口、主選單路由、共用工具 | `@BASE_INIT`, `@BASE_MENU`, `@SELECT_RESIDENT_NPC`, `@FACILITY_UPGRADE_MENU`, `@BASE_SYNC_DAILY_USES` |
| `BASE_LIFECYCLE.ERB` | 生命週期 Tick（晨間/晚間）、晨報 | `@BASE_EVENT_MORNING`, `@BASE_EVENT_NIGHT`, `@BASE_PRINT_MORNING_REPORT` |
| `BASE_UI.ERB` | HUD 與住宿 NPC 列表 UI 輸出 | `@BASE_PRINT_HUD`, `@BASE_PRINT_NPC_LIST` |
| `BASE_TRAIN.ERB` | 訓練室選單、訓練成長公式 | `@BASE_TRAIN_MENU`, `@SYS_TRAIN_APPLY` |
| `BASE_WORK.ERB` | 工坊選單、生產佇列操作 | `@BASE_WORK_MENU`, `@FACILITY_WORK_CRAFT_CARD` |
| `BASE_LOUNGE.ERB` | 休息室選單、互動前置檢查 | `@BASE_LOUNGE_MENU` |
| `BASE_INTERACT.ERB` | 互動系統完整封裝 | `@INTERACT_DISPATCH`, `@INTERACT_CMD_*`, `@CALC_INTERACT_*`, `@INTERACT_APPLY_*` |
| `BASE_ROOM.ERB` | 房間管理選單、入住/撤離唯一入口 | `@BASE_ROOM_MENU`, `@SYS_ROOM_MOVE_IN`, `@SYS_ROOM_MOVE_OUT`, `@ROOM_ASSIGN_MENU`, `@ROOM_EVICT_MENU` |

- 與戰鬥透過流程呼叫銜接（`CALL BATTLE_PREPARATION`），不直接承擔戰鬥結算細節
