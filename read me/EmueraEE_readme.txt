タイトル：Emuera。NET EM+EE 最終更新日:2025/11/14
バージョン：1.824+v24+EMv18+EEv55
改変者：Enter
元となったアプリケーション：Emuera1.824+v24（妊）|дﾟ)の中の人、及びMinorShift制作）、WebP-wrapper(JosePineiro制作)、Emuera.EM（EvilMask制作）、Emuera.NET（VVII制作）
連絡先：Twitter/@eraBEMANI Discord/https://discord.gg/p5rb5uK
docs：https://evilmask.gitlab.io/emuera.em.doc/
↑以下、説明が簡略な項については上記docsにて詳細に説明されています
...
　virustotal(ファイルの安全性確認サイト)のリンク：https://www.virustotal.com/gui/file/ed4019e34f54afb4d403051837f03221bebbe66629331055a67b0e16341913f5

[v12にてEmuera.EMと機能統合。上記リンクのドキュメント(docs)を参照]
[EMv8+EEv15にてhtml形式のドキュメントを同梱。追加機能などがより詳しく書かれています]
[v47にてEmuera.NETを一部マージ Emuera.NET→https://gitlab.com/VVIIlet/emuera]
...
・FLOWINPUT デフォルト値(, 左クリックをEnterキーとみなすか, 右クリックでスキップ可能か, 強制スキップ)
SHOW_SHOP内などフロー上で行われるINPUTにデフォルト値、クリックをEnterキーとみなすオプション、右クリックでスキップ可能のオプションを設定できる
EEv46で第四引数追加。非0の場合、デフォルト値をRESULTに入れて強制スキップする

・FLOWINPUTS 有効/無効(, デフォルト値)
非0で有効化。有効にするとシステムフロー上のINPUTが全てINPUTS扱いになる。SHOP画面等、入力値で処理が変わる場面ではFLOWINPUTでデフォルト値を設定することを推奨。その他の挙動制御はFLOWINPUTで行う
...
・SETBGIMAGE、REMOVEBGIMAGE、CLEARBGIMAGE
Neo_Kesha氏による実装。それぞれEmueraの背景画像についての命令

・CALLSHARP
Neo_Kesha氏による実装。C#をプラグインとして扱い、ERB上から呼び出せる

・CSVエイリアス機能
Neo_Kesha氏による実装。超ざっくり言うと拡張子「.als」ファイルでCSV変数に複数の識別子を与えることができる

・BITMAP_CACHE_ENABLE
ユケッス氏による実装。画面上の文字をビットマップとして描画することで処理を高速化する試み

・VARI,VARS
Emuera.NETからの移植。関数直下でなくとも変数を宣言できる

・PRINTN系
Emuera.NETからの移植。改行しないPRINTW系命令

・HTML_PRINT_ISLAND,HTML_PRINT_ISLAND_CLEAR
Emuera.NETからの移植。使い方は通常のHTML_PRINTと同じだが、ページスクロールに影響しない
後者はHTML_PRINT_ISLANDで表示した内容をクリアする

・HOTKEY_STATE,HOTKEY_STATE_INIT
ホットキーに関する制御系命令。詳しくはdocsで
...
・.NET 8に正式対応
fairylord(KFCの人)氏が対応してくれました。感謝
...
・特殊なコメントアウト追加
「;^;」から始まるコメントアウト行はeramaker及び別バージョンのEmueraでは通常通りコメントアウトされ、EMEEでのみ動作する
